...
public class OriPlanePositionHandler : MonoBehaviour
{
    public int gridRows = 3;
    public int gridColumns = 3;
    public float spacing = 2.0f;
    public GameObject planePrefab;
    public OriLinkingHandler OriLinkingHandler; // Reference to the LinkingHandler script
    ...
    void CreateAndArrangePlanes()
    {
        for (int row = 0; row < gridRows; row++)
        {
            for (int column = 0; column < gridColumns; column++)
            {
                ...
                {
                    OriLinkingHandler.UpdateLinePositions(newPlane.transform, position, spacing);
                }
            }
        }
    }
